  ...
  return (
    <Layout>
      <div className='max-w-5xl'>
        ...
        <P>Decent Checkout enables users to onramp to any token on any supported chain and complete transactions with fiat.  You can easily install customizable onramp links like the examples below to seamlessly onboard new users to your app.  Please see our docs for more information.</P>
        <P>Prompt a user to checkout and link to the appropriate Decent Checkout URL.  When the transaction completes, the user will be redirected back to your site.</P>
        <div className='py-8'>
          <div className='flex gap-4 items-center'>
              <a 
                href='https://checkout.decent.xyz/' target='_blank'
          ...
          <div className='flex gap-4 items-center'>
            <a 
              href={connectedAddress && `https://checkout.decent.xyz?wallet=${connectedAddress}`} target='_blank'
              className='px-4 py-2 rounded-md text-white bg-black'
            >
          ...
          <div className='flex gap-4 items-center'>
            <a 
              href={connectedAddress && `https://checkout.decent.xyz?wallet=${connectedAddress}&chain=${ChainId.ARBITRUM}&token=${ARB_MAGIC}&app=onramp`} target='_blank'
              className='px-4 py-2 rounded-md text-white bg-black'
            >
          ...
          <div className='flex gap-4 items-center'>
            <a 
              href={`https://checkout.decent.xyz?wallet=${connectedAddress}&chain=${ChainId.OPTIMISM}&app=nft&nft=${EARLY_OPTIMISTS}%3A${EARLY_OPTIMISTS_ID}&redir=${REDIRECT_URL}`} target='_blank'
              className='px-4 py-2 rounded-md text-white bg-black'
            >
          ...
          <div className='flex gap-4 items-center'>
            <a 
              href={`https://checkout.decent.xyz?wallet=${connectedAddress}&chain=${ChainId.ARBITRUM}&token=${ARB_MAGIC}&app=bridge`} target='_blank'
              className='px-4 py-2 rounded-md text-white bg-black'
            >
